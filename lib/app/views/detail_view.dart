...
class DetailView extends StatelessWidget {
  ...
  final LovedController _lc = Get.find<LovedController>();
  ...
  Widget _buildSliverHeader(BuildContext ctx, MovieModel movie) {
    return SliverAppBar(
      ...
      actions: [
        // Watchlist toggle
        Obx(() {
          final inList = _lc.isInWatchlist(movie.id);
          return GestureDetector(
            onTap: () => _lc.toggleWatchlist(movie),
            child: Container(
              margin: const EdgeInsets.all(8),
              ...
              child: Icon(
                ...
                color: inList ? AppTheme.accentGold : Colors.white,
                size: 22,
              ),
            ),
          );
          ...
          children: [
            Hero(
              ...
              child: CachedNetworkImage(
                imageUrl: movie.gambarSampul.isNotEmpty
                    ? movie.gambarSampul
                    : movie.gambarPoster,
                fit: BoxFit.cover,
    ...
    try {
      if (movie.tanggalRilis > 100000) {
        dateStr = DateFormat('dd MMM yyyy').format(
          DateTime.fromMillisecondsSinceEpoch(movie.tanggalRilis * 1000),
        );
      } else {
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: ElevatedButton.icon(
                              onPressed: () =>
                                  Get.toNamed('/admin-form', arguments: movie),
